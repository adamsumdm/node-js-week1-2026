...
async function readMembers(filePath) {
  ...
  // 檢核一：filePath 不得為 undefined
  if(!filePath) return [];
  try {
    const memberStream = await fs.readFile(filePath, 'utf-8');
    return JSON.parse(memberStream);
  } catch (error) {
...
function filterVIP(members) {
  ...
  return members.filter(m => m.level == 'VIP');
}
...
async function getVIPSummary(filePath) {
  ...
  //   2. 篩出 VIP
  const vipMembersList = filterVIP(members);
  //   3. 算總點數、收集姓名
  const totalCredits = sumCredits(vipMembersList);
  const names = [];
  vipMembersList.forEach(vip => names.push(vip.name));
  return{
    count:vipMembersList.length,
